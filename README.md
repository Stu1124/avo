...
Finder, Spotify, Google Drive, Claude Code and Codex, and any MCP server you add. Anything that
sends, creates, or changes something shows you an editable card first.

On macOS 26 you can skip the key and say **"Hey Avo"** instead. The phrase is matched on-device,
nothing leaves the Mac until it is heard, and it stays off until you turn it on in
...
*Nothing is sent, created or deleted until you have seen it. Every field is editable.*

<img src="docs/media/notch-answer.png" width="620" alt="A calendar answer in the notch">

*Read-only answers land in the notch and disappear on their own.*

<img src="docs/media/coding-tasks.png" width="380" alt="The side notch tracking two coding tasks">

*Long-running work — Claude Code, Codex — moves to the side notch so you can keep talking.*

## Requirements

- macOS 15 or newer, Apple Silicon. macOS 15 support is compile-verified; please report issues.
- The "Hey Avo" wake word needs macOS 26. Everything else works on the floor.
...
Signed downloads arrive with the first tagged release, on the [Releases](../../releases) page. An
...
Avo asks for four permissions during onboarding and everything else only when a tool actually needs
one. Live status for all of them is in **Settings → Permissions**.

Up front:

- **Microphone** — hears you while you hold the talk key. Required.
- **Speech Recognition** — turns speech into text, on this Mac. Required.
- **Input Monitoring** — sees the talk key while another app is in front. Can wait.
- **Accessibility** — reads your selected text and which app you are in. Can wait.
...
- **Full Disk Access** — iMessage history, and Desktop/Documents searches without per-folder
  prompts. macOS offers no in-app prompt for this one; add Avo in System Settings.
...
Nothing is requested at launch, and every feature degrades to a card explaining what is missing
rather than failing silently.

## Privacy

Dictation and wake-word detection run on-device, through Apple's speech models. Your audio is never
uploaded.
...
screen. It goes to the provider you configured and nowhere else — Avo has no backend. Point it at
Ollama or LM Studio and nothing leaves the machine at all.

Keys live in the macOS Keychain. History, screenshots and logs live in
`~/Library/Application Support/Avo/` and are swept on a retention window you set in Settings.
...
Their tools appear to the model as `mcp_<server>_<tool>` and get a confirmation card for anything
that writes. **Settings → Apps** lists every server with its transport, tool count or startup
error, an on/off switch, and a form for adding one without editing the file.
...
Avo answers a URL scheme, so anything that can open a URL can ask it a question:
